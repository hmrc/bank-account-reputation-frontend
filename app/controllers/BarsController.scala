          ...
          assess <- if (input.input.account.accountNumber.isDefined && metadata.isDefined) {
            input.input.account.accountType match {
              case Some("personal") => connector.assessPersonal(
                ...
                input.input.account.accountNumber.get,input.input.account.rollNumber, None,
        ...
        yield {
          val dataEvent = DataEvent(
            ...
            detail = Map[String, String](
              ...
              "AccountType" -> input.input.account.accountType.getOrElse("business")) ++ input.input.account.rollNumber.map(rn => Map("RollNumber" -> rn)).getOrElse(Map())
